...
#=====Unit Test=========
def stock_input_test(stock_symbol, chart_type, display_pref, start_date, end_date):
    
    #checks for symbol, capitalized, 1-7 alpha characters
    check_symbol(stock_symbol)

    #chart type, 1 numeric character, 1 or 2
    chart = int(chart_type)

    if chart < 1 or chart > 2:
        return False
    
    #time series: 1 nurmeric character, 1 - 4
    time = int(display_pref)
    if time < 1 or time > 4:
        return False
...
def check_symbol(stock_symbol):
    count = 0
    for _ in stock_symbol:
        count += 1
    
    if count < 1 or count > 7:
        return False
    
    for character in stock_symbol:
...
class UnitTest(unittest.TestCase):
    ...
    def test_One(self):
        self.assertTrue(stock_input_test('GOOG', '1', '3', '2024-05-05', '2024-06-05'))
